...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#060408] text-[#E8E2D8] font-sans pb-24 selection:bg-amber-500/30">
      ...
      {/* ========================================================================= */}
      {/* ========================================================================= */}
        {/* MODAL: CLEAR ALL SCHEDULED EMAILS CONFIRMATION DIALOG                      */}
        {/* ========================================================================= */}
        {showClearAllModal && (
          <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
            <div className="max-w-md w-full bg-[#120814] border border-red-500/40 rounded-2xl p-6 shadow-2xl space-y-5 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-red-500/10 rounded-full blur-3xl pointer-events-none" />

              <div className="flex items-start gap-3 border-b border-red-500/20 pb-4">
                <div className="w-10 h-10 rounded-full bg-red-950/60 border border-red-500/40 flex items-center justify-center text-red-400 flex-shrink-0">
                  <Trash2 className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-serif font-bold text-red-200">
                    Clear all scheduled emails?
                  </h3>
                  <p className="text-xs text-amber-200/80 mt-1 leading-relaxed">
                    This will permanently remove all currently scheduled email instances. Your reusable templates and settings will remain unchanged.
                  </p>
                </div>
              </div>

              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl space-y-1.5 text-[11px] text-amber-300">
                <div className="flex items-center gap-1.5 font-bold text-amber-200">
                  <ShieldCheck className="w-3.5 h-3.5 text-amber-400" /> Preserved Configuration:
                </div>
                <ul className="list-disc list-inside space-y-0.5 text-amber-300/80 pl-1">
                  <li>Master Email Templates (Advance 01–12, Birthday, Tithi)</li>
                  <li>Living Timeline & Tithi Dates (2003–2030)</li>
                  <li>Recipient Profiles (T1, S1–S5) & SMTP Configuration</li>
                </ul>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                <button
                  ...
                  className="px-4 py-2 bg-white/5 border border-white/10 text-amber-300/80 text-xs font-semibold rounded-lg hover:bg-white/10 hover:text-amber-200 transition-colors disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  onClick={handleClearAllScheduledEmails}
                  disabled={isClearingAll}
                  className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white font-bold text-xs rounded-lg transition-all shadow-lg shadow-red-900/30 flex items-center gap-1.5 disabled:opacity-50"
                >
                  {isClearingAll ? (
                    <>
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" /> Clearing Database...
                    </>
                  ) : (
                    <>
